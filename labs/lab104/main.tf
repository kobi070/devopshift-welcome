...
resource "random_shuffle" "az_selector" {
  input        = var.az
  result_count = 1
}
...
data "aws_subnets" "default" {
  ...
  filter {
    ...
    values = [random_shuffle.az_selector.result[0]]
  }
}
...
output "network_info" {
  value = var.create_vpc ? "The following is your VPC ID: ${aws_vpc.custom_vpc[0].id} and Subnet ID: ${aws_subnet.custom_subnet[0].id}" : "Using Default VPC, Subnet ID: ${data.aws_subnet.default.id}"
}